    ...
    <div className="pyro-input-container">
      ...
      <div className='pyro-input-field-container' >
        ...
        <ul className={`select ${open? 'open': 'closed'}`}>
          {matchingItems.map(({displayName, value}, index) => <li key={value} value={value} onClick={handleOptionClick} className={`select-item ${arrowFocus === index? "arrowFocused": ""}`}>{displayName}</li>)}
        </ul>
